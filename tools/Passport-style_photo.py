...
def run():
    ...
    st.set_page_config(
        page_title="Passport-style photo generator"
    )

    st.title("Passport-style Photo Generator")

    st.write(
        "Load a photograph and generate an image with "
    ...
    if archivo is not None:
        ...
        st.write(
            f"Size: {ancho_cm} × {alto_cm} cm "
            f"({ancho_px} × {alto_px} px at 300 DPI)"
        )
